...
int main(int arc, char **argv) {
    ...
    pSuite1 = CU_add_suite("Testing util functions", NULL, NULL);
    if (!pSuite1) {
        goto exit;
    }
...
void test_get_jump_offset() {
    Instruction inst = parse_instruction(0x004009ef);
    int offset = get_jump_offset(inst);
    CU_ASSERT_EQUAL(offset, 4);
    inst = parse_instruction(0x00800a6f);
    offset = get_jump_offset(inst);
    CU_ASSERT_EQUAL(offset, 8);
}

void test_get_branch_offset() {
    Instruction inst = parse_instruction(0x00711263);
    int offset = get_branch_offset(inst);
    CU_ASSERT_EQUAL(offset, 4);
    inst = parse_instruction(0x00cd9263);
    offset = get_branch_offset(inst);
    CU_ASSERT_EQUAL(offset, 4);
}

void test_get_store_offset() {
    Instruction inst = parse_instruction(0x51201d23);
    int offset = get_store_offset(inst);
    CU_ASSERT_EQUAL(offset, 1306);
    inst = parse_instruction(0x2f201323);
    offset = get_store_offset(inst);
    CU_ASSERT_EQUAL(offset, 742);
}
